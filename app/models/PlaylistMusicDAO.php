...
class PlaylistMusicDAO
{
    ...
    public function addMusicToPlaylist(PlaylistMusic $playlistMusic)
    {
//        $idPlaylist = $playlistMusic->getPlaylist()->getId();
//        $idmusic = $playlistMusic->getMusic()->getId();

        try {
            $query = "INSERT INTO playlists_music VALUES (?,?)";
            $statement = $this->conn->prepare($query);
            $statement->bindParam(1, $idmusic, PDO::PARAM_INT);
            $statement->bindParam(2, $idPlaylist, PDO::PARAM_INT);
